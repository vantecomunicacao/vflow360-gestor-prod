  ...
  try {
    ...
    if (isOpenAI) {
      ...
      try {
        ...
        return text ? `🎵 [Áudio]: ${text}` : "[🎵 Áudio recebido]";
      } catch (e) {
  ...
  try {
    ...
    if (media) {
      ...
      if ((media.type === "audio" || media.type === "image") && aiKey) {
        ...
        content = vid?.caption ? `🎬 [Vídeo]: ${vid.caption}` : "[🎬 Vídeo recebido]";
        ...
        if (isPdf) {
          ...
          if (pdfBase64 && pdfBase64.length > 100) {
            try {
              ...
              if (!pdfResp.ok) {
                console.error("Stevo PDF extract HTTP", pdfResp.status, await pdfResp.text());
...
serve(async (req) => {
  ...
  try {
    ...
    // HMAC opt-in: depende do provedor enviar X-Signature (default).
    // Header e formato podem ser sobrescritos via env STEVO_HMAC_HEADER.
    const rawBody = await req.text();
    const hmac = await verifyWebhookHmac(
      { prefix: "STEVO", defaultHeader: "x-signature", format: "hex" },
      req,
      rawBody,
      "edge:stevo-webhook",
    );
    ...
    await reportEdgeError("edge:stevo-webhook", error);
    return okResponse;
  }
});
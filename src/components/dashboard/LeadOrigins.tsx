...
const COLORS = [
  "hsl(215, 50%, 45%)", "hsl(160, 60%, 42%)", "hsl(38, 92%, 50%)", "hsl(262, 52%, 47%)",
  "hsl(340, 65%, 47%)", "hsl(200, 60%, 50%)", "hsl(152, 60%, 45%)", "hsl(0, 0%, 60%)",
];

interface LeadOriginsProps { leadOrigins: LeadOrigin[]; fillRate: number; }

export function LeadOrigins({ leadOrigins, fillRate }: LeadOriginsProps) {
  const origins = leadOrigins || [];
    ...
    <div className="dashboard-section animate-slide-up h-full">
      ...
      {origins.length === 0 ? (
        <div className="flex items-center justify-center h-48">
          <p className="text-muted-foreground text-sm text-center">Nenhuma origem preenchida no período.</p>
        </div>
      ) : (
